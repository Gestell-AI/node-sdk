...
export default function JobService(client: GestellOptions = {}): JobServiceApi {
  ...
  async function get(payload: GetJobRequest) {
    return getJob({ ...options, ...payload })
  }

  /** Wraps {@link getJobs} with injected client options. */
  async function list(payload: GetJobsRequest) {
    return getJobs({ ...options, ...payload })
  }

  /** Wraps {@link reprocessDocuments} with injected client options. */
  async function reprocessFn(payload: ReprocessDocumentsRequest) {
    return reprocessDocuments({ ...options, ...payload })
  }

  /** Wraps {@link cancelJobs} with injected client options. */
  async function cancelFn(payload: CancelJobsRequest) {
    return cancelJobs({ ...options, ...payload })
  }
