import { LayoutType } from '@gestell/types/layout'

/**
 * Types of processing steps available for a job.
 */
export type JobType =
  | /** The OCR job status. */ 'status'
  | /** Canonization and node extraction step. */ 'nodes'
  | /** Vector generation step. */ 'vectors'
  | /** Edge linking step. */ 'edges'
  | /** Category assignment step. */ 'category'

/**
 * Possible states of a processing step or job.
 */
export type JobStatus =
  | /** Currently in progress. */ 'processing'
  | /** Encountered an error. */ 'error'
  | /** Completed successfully. */ 'ready'
  | /** Manually cancelled. */ 'cancelled'
  | /** Not yet started. */ 'unprocessed'
  | /** Partially completed. */ 'partial'
  | /** All steps completed. */ 'all'
